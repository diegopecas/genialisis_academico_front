...
@Component({
  selector: 'app-reporte-inventario-diario',
  templateUrl: './reporte-inventario-diario.component.html',
  styleUrl: './reporte-inventario-diario.component.scss',
  standalone: true,
  imports: [CommonModule, FormsModule, HeaderComponent, TablasComponent]
})
export class ReporteInventarioDiarioComponent implements OnInit {

  titulo = "Reporte de Inventario Diario";
  public columnasFiltro = ['Estudiante', 'Elemento', 'Grupo'];
  ...
  crearTitulos() {
    this.titulos = [
      ...
      { clave: 'elemento', alias: 'Elemento', alinear: 'izquierda' },
  ...
  consultar() {
    ...
    this.inventarioDiarioService.obtenerReporte(filtros).subscribe({
      next: (respuesta: any) => {
