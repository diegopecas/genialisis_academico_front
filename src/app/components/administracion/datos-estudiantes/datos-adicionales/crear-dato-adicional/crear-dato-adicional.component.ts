...
@Component({
  selector: 'app-crear-dato-adicional',
  templateUrl: './crear-dato-adicional.component.html',
  styleUrl: './crear-dato-adicional.component.scss',
  standalone: true,
  imports: [CommonModule, FormsModule, HeaderComponent]
})
export class CrearDatoAdicionalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(params => {
      ...
      if (this.accion === 'crear') {
        this.titulo = "Crear Dato Adicional";
        this.editable = true;
      } else if (this.accion === 'editar') {
        this.titulo = "Editar Dato Adicional";
        this.editable = true;
        this.cargarRegistro(id);
      } else if (this.accion === 'consultar') {
        this.titulo = "Consultar Dato Adicional";
        this.editable = false;
        this.cargarRegistro(id);
      }
    });
  ...
  cargarTipos() {
    this.tiposDatosAdicionalesService.obtenerTodos().subscribe({
      next: (response: any) => {
  ...
  cargarRegistro(id: any) {
    this.datosAdicionalesService.obtenerById(id).subscribe({
      next: (response: any) => {
        const body = response.body;
        if (body && body.length > 0) {
          const reg = body[0];
          this.model.id = reg.id;
          this.model.id_tipo_dato_adicional = reg.id_tipo_dato_adicional;
          ...
          if (this.accion === 'editar') {
            this.titulo = "Editar Dato Adicional: " + this.model.nombre;
          } else if (this.accion === 'consultar') {
            this.titulo = "Consultar Dato Adicional: " + this.model.nombre;
          }
        }
      },
      error: () => {
  ...
  guardar() {
    ...
    if (!this.model.id_tipo_dato_adicional) {
      Swal.fire('Advertencia', 'Seleccione una categoría', 'warning');
      return;
    }

    const data = {
      id_tipo_dato_adicional: this.model.id_tipo_dato_adicional,
    ...
    if (this.accion === 'crear') {
      this.datosAdicionalesService.crear(data).subscribe({
        next: () => {
          Swal.fire('Éxito', 'Dato adicional creado correctamente', 'success');
          this.router.navigate([this.regresar]);
        },
        error: () => {
          Swal.fire('Error', 'No se pudo crear el dato adicional', 'error');
        }
      });
    } else if (this.accion === 'editar') {
      data.id = this.model.id;
      this.datosAdicionalesService.actualizar(data).subscribe({
        next: () => {
          Swal.fire('Éxito', 'Dato adicional actualizado correctamente', 'success');
          this.router.navigate([this.regresar]);
        },
        error: () => {
          Swal.fire('Error', 'No se pudo actualizar el dato adicional', 'error');
        }
      });
    }
  }
